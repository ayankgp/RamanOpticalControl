...
if __name__ == '__main__':
    ...
    absorption_GEVI = savgol_filter(absorption_GEVI, 15, 3)
    ...
    # prob_GEVI = np.asarray([0.0375477, 0.150588, 0.417190, 0.685970, 0.989216, 1.000000, 0.999991, 0.802483, 0.5997950, 0.4237150, 0.3512940])  # GEVI-updated
    ...
    params = ADict(
        ...
        max_iter=1,
        control_guess=np.asarray([0.00029131, 0.000205391, 0.0234365, 0.00728493, 0.0840182, 600, 9403.24, 63138.3]),  # GECI-GEVI-----18.75
        ...
        control_upper=np.asarray([0.001, 0.001, 1.15 * energy_factor, Raman_levels_GECI[3] * 1.010, 1239.84 * energy_factor / 496.5, 600, 11000, 75000]),
    ...
    fig.canvas.set_window_title('GCaMP-ASAP')
    ...
    params.control_guess = np.asarray([0, 0.000499275, 0.0343182, 0.00727424, 0.0909774, 600, 7441.36, 0.1])
    ...
    dyn_plot(axes[2, 0], time_R, time_A, molecule.dyn_rho_R_GEVI.real, molecule.dyn_rho_A_GEVI.real, '')
    ...
    axes[2, 0].set_ylabel("Population \n of GEVI", fontweight='bold', fontsize='medium')
    ...
    params.control_guess = np.asarray(
        [0.000282069, 0.000256575, 0.0178896, 0.00716119, 0.0851399, 600, 6452.18, 68388.7])
    params.control_lower = np.asarray(
        [0.0001, 0.0001, 0.35 * energy_factor, Raman_levels_GEVI[3] * 0.990, 1239.84 * energy_factor / 550.5, 600, 5000,
         50000])
    params.control_upper = np.asarray(
        [0.001, 0.001, 1.15 * energy_factor, Raman_levels_GEVI[3] * 1.010, 1239.84 * energy_factor / 476.5, 600, 7500,
         75000])
    ...
    plt.savefig('FinalPaperPlots/GECI_GEVI_dynamics.eps', format="eps")
    plt.savefig('FinalPaperPlots/GECI_GEVI_dynamics.png', format="png")
    plt.show()